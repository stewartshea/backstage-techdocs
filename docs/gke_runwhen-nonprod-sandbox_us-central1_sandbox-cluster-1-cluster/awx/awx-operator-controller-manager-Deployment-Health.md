---
title: "awx-operator-controller-manager Deployment Health  "
tags: 
...
# awx-operator-controller-manager Deployment Health    
...
!!! note ""
    ...
    #### Troubleshoot Deployment Warning Events for `awx-operator-controller-manager`  
    </div>
    !!! warning ""
    ...
    <div class="code-block-container">
      <pre class="code-block"><code id="codeBlock1">```kubectl get events --context gke_runwhen-nonprod-sandbox_us-central1_sandbox-cluster-1-cluster -n awx -o json | jq '(now - (60*60)) as $time_limit | [ .items[] | select(.type == "Warning" and (.involvedObject.kind == "Deployment" or .involvedObject.kind == "ReplicaSet" or .involvedObject.kind == "Pod") and (.involvedObject.name | tostring | contains("awx-operator-controller-manager")) and (.lastTimestamp | fromdateiso8601) >= $time_limit) | {kind: .involvedObject.kind, name: .involvedObject.name, reason: .reason, message: .message, firstTimestamp: .firstTimestamp, lastTimestamp: .lastTimestamp} ] | group_by([.kind, .name]) | map({kind: .[0].kind, name: .[0].name, count: length, reasons: map(.reason) | unique, messages: map(.message) | unique, firstTimestamp: map(.firstTimestamp | fromdateiso8601) | sort | .[0] | todateiso8601, lastTimestamp: map(.lastTimestamp | fromdateiso8601) | sort | reverse | .[0] | todateiso8601})'```</code></pre>
    </div>
    <div class="grid-item">
...
!!! note ""
    <div class="command-title">
    #### Get Deployment Workload Details For `awx-operator-controller-manager` and Add to Report  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
    ...
    <div class="code-block-container">
      <pre class="code-block"><code id="codeBlock2">```kubectl get deployment/awx-operator-controller-manager --context gke_runwhen-nonprod-sandbox_us-central1_sandbox-cluster-1-cluster -n awx -o yaml```</code></pre>
    </div>
    <div class="grid-item">
...
!!! note ""
    <div class="command-title">
    #### Troubleshoot Deployment Replicas for `awx-operator-controller-manager`  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
...
!!! note ""
    <div class="command-title">
    #### Check Deployment Event Anomalies for `awx-operator-controller-manager`  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
...
document.getElementById('configLink').addEventListener('click', function() {
    showConfig('/workspaces/ws/slxs/aoc-awx-grnwhnnpr-depl-health/runbook.yaml');
});
...
function showConfig(runbook) {
    ...
    fetch('/get-runbook-config', {
        ...
        .then(data => {
            ...
            title.innerText = "Private configuration for: " + 'awx-operator-controller-manager Deployment Health  ';
