---
title: "loki-grafana-agent-operator Deployment Health  "
tags: 
...
# loki-grafana-agent-operator Deployment Health    
...
!!! note ""
    ...
    #### Troubleshoot Deployment Warning Events for `loki-grafana-agent-operator`  
    </div>
    !!! warning ""
    ...
    <div class="code-block-container">
      <pre class="code-block"><code id="codeBlock1">```kubectl get events --context gke_runwhen-nonprod-sandbox_us-central1_sandbox-cluster-1-cluster -n loki -o json | jq '(now - (60*60)) as $time_limit | [ .items[] | select(.type == "Warning" and (.involvedObject.kind == "Deployment" or .involvedObject.kind == "ReplicaSet" or .involvedObject.kind == "Pod") and (.involvedObject.name | tostring | contains("loki-grafana-agent-operator")) and (.lastTimestamp | fromdateiso8601) >= $time_limit) | {kind: .involvedObject.kind, name: .involvedObject.name, reason: .reason, message: .message, firstTimestamp: .firstTimestamp, lastTimestamp: .lastTimestamp} ] | group_by([.kind, .name]) | map({kind: .[0].kind, name: .[0].name, count: length, reasons: map(.reason) | unique, messages: map(.message) | unique, firstTimestamp: map(.firstTimestamp | fromdateiso8601) | sort | .[0] | todateiso8601, lastTimestamp: map(.lastTimestamp | fromdateiso8601) | sort | reverse | .[0] | todateiso8601})'```</code></pre>
    </div>
    <div class="grid-item">
...
!!! note ""
    <div class="command-title">
    #### Get Deployment Workload Details For `loki-grafana-agent-operator` and Add to Report  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
    ...
    <div class="code-block-container">
      <pre class="code-block"><code id="codeBlock2">```kubectl get deployment/loki-grafana-agent-operator --context gke_runwhen-nonprod-sandbox_us-central1_sandbox-cluster-1-cluster -n loki -o yaml```</code></pre>
    </div>
    <div class="grid-item">
...
!!! note ""
    <div class="command-title">
    #### Troubleshoot Deployment Replicas for `loki-grafana-agent-operator`  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
...
!!! note ""
    <div class="command-title">
    #### Check Deployment Event Anomalies for `loki-grafana-agent-operator`  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
...
document.getElementById('configLink').addEventListener('click', function() {
    showConfig('/workspaces/ws/slxs/lga-lk-grnwhnnpr-depl-health/runbook.yaml');
});
...
function showConfig(runbook) {
    ...
    fetch('/get-runbook-config', {
        ...
        .then(data => {
            ...
            title.innerText = "Private configuration for: " + 'loki-grafana-agent-operator Deployment Health  ';
