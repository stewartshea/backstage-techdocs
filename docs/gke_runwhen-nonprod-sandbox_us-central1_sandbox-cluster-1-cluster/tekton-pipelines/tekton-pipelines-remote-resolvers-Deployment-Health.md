---
title: "tekton-pipelines-remote-resolvers Deployment Health  "
tags: 
...
# tekton-pipelines-remote-resolvers Deployment Health    
...
!!! note ""
    ...
    #### Troubleshoot Deployment Warning Events for `tekton-pipelines-remote-resolvers`  
    </div>
    !!! warning ""
    ...
    <div class="code-block-container">
      <pre class="code-block"><code id="codeBlock1">```kubectl get events --context gke_runwhen-nonprod-sandbox_us-central1_sandbox-cluster-1-cluster -n tekton-pipelines -o json | jq '(now - (60*60)) as $time_limit | [ .items[] | select(.type == "Warning" and (.involvedObject.kind == "Deployment" or .involvedObject.kind == "ReplicaSet" or .involvedObject.kind == "Pod") and (.involvedObject.name | tostring | contains("tekton-pipelines-remote-resolvers")) and (.lastTimestamp | fromdateiso8601) >= $time_limit) | {kind: .involvedObject.kind, name: .involvedObject.name, reason: .reason, message: .message, firstTimestamp: .firstTimestamp, lastTimestamp: .lastTimestamp} ] | group_by([.kind, .name]) | map({kind: .[0].kind, name: .[0].name, count: length, reasons: map(.reason) | unique, messages: map(.message) | unique, firstTimestamp: map(.firstTimestamp | fromdateiso8601) | sort | .[0] | todateiso8601, lastTimestamp: map(.lastTimestamp | fromdateiso8601) | sort | reverse | .[0] | todateiso8601})'```</code></pre>
    </div>
    <div class="grid-item">
...
!!! note ""
    <div class="command-title">
    #### Get Deployment Workload Details For `tekton-pipelines-remote-resolvers` and Add to Report  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
    ...
    <div class="code-block-container">
      <pre class="code-block"><code id="codeBlock2">```kubectl get deployment/tekton-pipelines-remote-resolvers --context gke_runwhen-nonprod-sandbox_us-central1_sandbox-cluster-1-cluster -n tekton-pipelines -o yaml```</code></pre>
    </div>
    <div class="grid-item">
...
!!! note ""
    <div class="command-title">
    #### Troubleshoot Deployment Replicas for `tekton-pipelines-remote-resolvers`  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
...
!!! note ""
    <div class="command-title">
    #### Check Deployment Event Anomalies for `tekton-pipelines-remote-resolvers`  
    </div>
    !!! warning ""
    <div class="command-details">
    What does it do?
...
document.getElementById('configLink').addEventListener('click', function() {
    showConfig('/workspaces/ws/slxs/tpr-tp-grnwhnnpr-depl-health/runbook.yaml');
});
...
function showConfig(runbook) {
    ...
    fetch('/get-runbook-config', {
        ...
        .then(data => {
            ...
            title.innerText = "Private configuration for: " + 'tekton-pipelines-remote-resolvers Deployment Health  ';
